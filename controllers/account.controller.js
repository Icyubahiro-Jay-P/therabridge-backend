...
export const exportMyData = async (req, res) => {
  try {
    ...
    const [
      ...
      Assessment.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      GratitudeEntry.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      Activity.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      CopingCard.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      PsychoedProgress.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      UserProgress.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      SleepLog.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      Medication.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      MedicationLog.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      Pet.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      JournalEntry.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      Habit.find({ user: userId }).sort({ createdAt: 1 }).lean(),
      HabitLog.find({ user: userId }).sort({ createdAt: 1 }).lean(),
    ]);
    ...
    const data = {
      ...
      safetyPlan: safetyPlan
        ...
        : null,
      thoughtRecords: thoughtRecords.map((t) => ({
        ...t,
        situation: decryptField(t.situation),
        automaticThought: decryptField(t.automaticThought),
        emotions: decryptField(t.emotions),
        evidenceFor: decryptField(t.evidenceFor),
        evidenceAgainst: decryptField(t.evidenceAgainst),
        reframe: decryptField(t.reframe),
        outcomeEmotion: decryptField(t.outcomeEmotion),
      })),
      assessments,
      gratitudes: gratitudes.map((g) => ({ ...g, content: decryptField(g.content) })),
      activities: activities.map((a) => ({
        ...a,
        title: decryptField(a.title),
        notes: a.notes ? decryptField(a.notes) : "",
      })),
      copingCards: copingCards.map((c) => ({ ...c, text: decryptField(c.text) })),
      psychoedProgress,
      programProgress,
      sleepLogs: sleepLogs.map((s) => ({
        ...s,
        notes: decryptField(s.notes),
        dreams: decryptField(s.dreams),
      })),
      medications: medications.map((m) => ({
        ...m,
        name: decryptField(m.name),
        dosage: decryptField(m.dosage),
        notes: m.notes ? decryptField(m.notes) : null,
      })),
      medicationLogs: medicationLogs.map((l) => ({
        ...l,
        notes: l.notes ? decryptField(l.notes) : null,
      })),
      pets,
      journalEntries: journalEntries.map((j) => ({
        ...j,
        title: decryptField(j.title),
        content: decryptField(j.content),
        comments: (j.comments || []).map((c) => ({ ...c, content: decryptField(c.content) })),
      })),
      habits: habits.map((h) => ({ ...h, name: decryptField(h.name) })),
      habitLogs,
      recordCounts: {
        ...
        thoughtRecords: thoughtRecords.length,
        assessments: assessments.length,
        gratitudes: gratitudes.length,
        activities: activities.length,
        copingCards: copingCards.length,
        psychoedProgress: psychoedProgress.length,
        programProgress: programProgress.length,
